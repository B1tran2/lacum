...
function mapNoteToPlaybackEvent(note: {
  trackId: string;
  pitch: number;
  startTicks: number;
...
export default function ProjectPage() {
  ...
  const fullProjectNotes = useMemo(() => {
    if (!data) {
      return [];
    }
    ...
    return [...data.midi.notes]
      ...
      .map(mapNoteToPlaybackEvent);
  }, [data]);

  const previewNotes = useMemo(
    () => fullProjectNotes.slice(0, MAX_PREVIEW_NOTES),
    [fullProjectNotes],
  );
  ...
  const handleExportWav = async () => {
    ...
    try {
      const blob = await renderProjectToWav(fullProjectNotes, data.global.bpm, trackMixer);
